...
| A pre-send control exists but has no usable layout box | Treat the page as malformed: close, reload, reopen at position 1, and resume by userId. Two consecutive recovery reloads are allowed; a persistently broken page stops. Never recover this way while a send is unresolved. |
