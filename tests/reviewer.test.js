...
// What the page does, reduced to the four facts the plan established live on
// 2026-08-12 and nothing more:
//   - one click on `View application` opens the reviewer;
...
//   - `ArrowRight` moves on without acting, leaving M alone.
...
function createPage(options = {}) {
  ...
  function render() {
    ...
    document.body.innerHTML =
      '<button id="open">View application</button>' +
      (state.open
        ? '<div role="dialog">' +
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'ArrowRight' && state.index < state.queue.length) {
      state.index += 1;
      render();
    }
  });
...
describe('opening the reviewer', () => {
  it('opens it with a single click and no retry', () => {
    start();
    expect(driver.openReviewer()).toEqual({ opened: true });
    expect(page.state.open).toBe(true);
...
describe('reporting who is shown', () => {
  it('reads the id from the resume link and the position from the counter', () => {
    start();
    driver.openReviewer();
    expect(driver.readCurrent()).toEqual({ userId: '21527289', index: 1, total: 3 });
  });

  it('errors rather than returning null when the id cannot be read', () => {
    start({ queue: [] });
    driver.openReviewer();
  ...
  it('errors when the modal carries two different candidate ids', () => {
    start({ extraMarkup: '<a href="/link/99999999/tokX/resume_url">Other</a>' });
    driver.openReviewer();
...
describe('the never-reject guard', () => {
  it('refuses to send the reject keys', () => {
    start();
    driver.openReviewer();
  ...
  it('refuses to click a reject control even when handed one directly', () => {
    start();
    driver.openReviewer();
...
describe('the exactly-one-control guard', () => {
  ...
  it('aborts when the only match is disabled', () => {
    start();
    driver.openReviewer();
...
describe('skipping', () => {
  it('advances with the next key, leaving the bucket alone', async () => {
    start();
    driver.openReviewer();
    const next = await driver.skipCurrent();
    expect(page.state.keys).toEqual(['ArrowRight']);
  ...
  it('reports rather than hanging when the reviewer does not move', async () => {
    ...
    const settled = expect(pending).rejects.toThrow(/did not move to the next candidate/);
    await vi.advanceTimersByTimeAsync(15000);
    await settled;
  });
});
...
describe('the message handlers', () => {
  it('answers the four reviewer types and ignores everything else', async () => {
    ...
    expect(answers()).toEqual([
      { source: 'wfx-page', id: 'a1', ok: true, data: { opened: true } },
      {
        source: 'wfx-page',
        id: 'a2',
        ok: true,
